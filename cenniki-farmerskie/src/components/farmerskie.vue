<template>
  <div class="hello">
    <div class="cennikkorner">
      ...
      <div v-for="row in datafarmerskie" :key="row.id" class="row lista">
        <div class="col-3 model">
          <span v-if="row.nazwa">{{ row.nazwa }}</span>
        </div>
        <div class="col-3">
...
export default {
  ...
  methods: {
    getfarmerskie() {
      this.$http
        .post("ogrodzenia/farmerskie", JSON.stringify({}), {})
        .then((response) => {
          this.datafarmerskie = response.body;
          this.loading = false;
        });
    },
  },
